...
public class FakerDataGenerator {
	...
	public static CreateJobPayload generateFakeCreateJobData() {
		...
		CreateJobPayload payload = new CreateJobPayload(MST_SERVICE_LOCATION_ID, MST_PLATEFORM_ID, MST_WARRANTY_STATUS_ID, MST_OEM_ID,
	...
	@Step("Generating Fake Proble List for the create Job payload")    
	private static List<Problems> generateFakeProblemsList() {
		...
		for(int i=1; i<=count; i++) {
		//Generating a random Problems ID and adding it to the list
		randomIndex=RANDOM.nextInt(validProblemsId.length);
		fakerRemark = faker.lorem().sentence(10);
	...
	private static CustomerProduct generateFakeCustomerProductData() {
		...
		CustomerProduct customerProduct = new CustomerProduct(dop, imeiSerialNumber,
				imeiSerialNumber, imeiSerialNumber, popUrl, PRODUCT_ID, MST_MODEL_ID);
